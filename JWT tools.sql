...
with t(jwt) as 
(
 select 'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzM4NCJ9.'
        'eyJpc3MiOiJCYWogU3RlZmFuIiwiaWF0IjoxNzM5NTU5OTc1LCJleHAiOjE3NzExMDEzNDcsImF1ZCI6IiIsInN1YiI6IiIsIkdpdmVuTmFtZSI6IkJhaiIsIlN1cm5hbWUiOiJTdGVmYW4ifQ.'
        'cYkdM4J42_d61Bz5ecgxu_mcp-baeoAitgNr7VmtWddgkMXeO_Wn1E_sQYEyyJv9'
),
det as
(
 select	svc.verify_jwt(jwt, 'Baba123Meca') as is_valid,
        svc.jwt_payload(jwt) as payload
 from t		
)
select is_valid, 
       payload->>'iss' as issuer, 
       to_timestamp((payload->>'exp')::integer) as expires 
from det;
